...
class ExportToBlackboard{
	...
	function build1Body(){
		...
		foreach($this->questions as $question){
			...
			$body .= $this->buildQuestion($question, $addJS);
			$idx++;
		}			
		return $body;
	}
	function buildQuestion($question, $addJS){
		$type = $this->getType($question);
		if ($type=='Multiple Choice') {
			return $this->buildQuestionMC($question, $addJS);
		} else if ($type=='Multiple Answer') {
			return $this->buildQuestionMCC($question, $addJS);
		} else if ($type=='Short Response') {
			return $this->buildQuestionSA($question, $addJS);
		} else {
			return '';
		}
	}
	function getType($question) {
		if ($question["elements"]['text_options']['1']['is_multiple_choice']) {
			return 'Multiple Choice';
		} else if ($question["elements"]['text_options']['1']['is_multiple_multiple_choice']) {
			return 'Multiple Answer';
		} else if ($question["elements"]['text_options']['1']['is_short_answer']) {
			return 'Short Response';
		} else {
			return 'Invalid type';
		}
	}
	function buildQuestionMC($question, $addJS) {
		$type = $this->getType($question);
		$text = '';
		$text .= $this->buildPrompt($question, $type, $addJS);
		...
		foreach($question["elements"]["answers_and_comments"] as $choice){
			...
			$choiceText .= $this->buildChoiceFlow($choice, $newId, $question, $type);	
			if($choice["is_correct"]){
				$reprocTextCorrect .= $this->buildReprocCorrectMC($newId);
			}
	...
	function buildQuestionMCC($question, $addJS) {
	
		$type = $this->getType($question);
		$text = '';
		$text .= $this->buildPrompt($question, $type, $addJS);
		$choiceText = '<flow class="RESPONSE_BLOCK"><response_lid ident="response" rcardinality="Multiple" rtiming="No"><render_choice maxnumber="0" minnumber="0" shuffle="No">'."\n";
		$reprocHeader = $this->buildReprocHeader();
		$reprocTextCorrect = '<respcondition title="correct"><conditionvar><and>
		';
		$reprocTextIncorrect = $this->buildReprocIncorrect();
		$feedbackText = '';
		$feedbackTextCorrect = $this->buildFeedbackCorrect();
		...
		foreach($question["elements"]["answers_and_comments"] as $choice){
			...
			array_push($qIds, $newId);
			$choiceText .= $this->buildChoiceFlow($choice, $newId, $question, $type);	
			$reprocTextCorrect .= $this->buildReprocMCC($newId, $choice["is_correct"]);
			
			$feedbackText .= $this->buildFeedback($newId);
			$choiceIdx++;
		}
		$reprocTextCorrect .= $this->buildReprocTextCorrectFooterMCC();
		$choiceText .= '</render_choice></response_lid></flow></flow></presentation>'."\n";
		$reprocFooter = '</resprocessing>'."\n";
		$text .= $choiceText;
		$text .= $reprocHeader . $reprocTextCorrect . $reprocTextIncorrect .  $reprocFooter;
		$text .= $feedbackTextCorrect . $feedbackTextIncorrect . $feedbackText;
		$text .= '</item>'."\n";
		return $text;	
	}
	function buildQuestionSA($question, $addJS) {
		$type = $this->getType($question);
		$text = '';
		$text .= $this->buildPrompt($question, $type, $addJS);
		$text .= $this->restOfSA();
	...
	function buildPrompt($question, $type, $addJS){
		//$type is Multiple Choice, Multiple Answer, or Short Response
		...
		if($question["elements"]["main_picture"]){
			$imgHTML = $this->toValidXML('<br><img src="'.$html_tool->GetPictureUrl($question["elements"]["main_picture"]).'"></img><br>');
		} else {
			$imgHTML = '';	
		}
		$prompt = '<item '."\n";
		...
		$prompt .= '<bbmd_questiontype>' . $type . '</bbmd_questiontype><bbmd_is_from_cartridge>false</bbmd_is_from_cartridge><bbmd_is_disabled>false</bbmd_is_disabled><bbmd_numbertype>none</bbmd_numbertype><bbmd_partialcredit>false</bbmd_partialcredit><bbmd_orientationtype>vertical</bbmd_orientationtype><bbmd_is_extracredit>false</bbmd_is_extracredit> '."\n";
		...
		$prompt .= 'type="HTML">';
		if ($addJS) {
			$prompt .= $this->toValidXML($this->buildJS());
		}
		if ($type=='Short Response') {
			$prompt .= $this->toValidXML('<div name="questionIdCWShortResponse" style="visibility:hidden;">'.$question["elements"]["question_id"].'</div>');
		}
		$prompt .= $this->toValidXML($question["elements"]["text_options"]["1"]["text"]) . $imgHTML . '</mat_formattedtext></mat_extension></material></flow>
		';
	...
	function buildChoiceFlow($choice, $Id, $question, $type){
		$html_tool = new DynamicPageMaker();
		if($choice["picture_id"]){//if there is answer picture
			$imgHTML = $this->toValidXML('<br><img src="'.$html_tool->GetPictureUrl($choice["picture_id"]).'"></img><br>');
		} else {
			$imgHTML = '';	
		}
		...
		$text .= 'type="HTML">'.$this->toValidXML($choice["text"]).$imgHTML.$this->addIdentifierDiv($question, $choice, $type).'</mat_formattedtext></mat_extension></material></flow_mat>
		';

		$text .= '</response_label></flow_label> '."\n";
		return $text;
	}
	function addIdentifierDiv($question, $choice, $type) {
		return $this->toValidXML('<div name="questionIdCW" style="visibility:hidden;">'.$question["elements"]["question_id"].'</div><div name="answerIdCW" style="visibility:hidden;">'.$choice["question_answer_id"].'</div><div name="questionTypeCW" style="visibility:hidden;">'.$type.'</div>');
	}
	...
	function buildReprocCorrectMC($Id){
		$text = '';
	...
	function buildReprocMCC($Id, $isCorrect){
		if ($isCorrect) {
			return '<varequal case="No" respident="response">' . $Id . '</varequal>
			';
		} else {
			return '<not><varequal case="No" respident="response">' . $Id . '</varequal></not>
			';
		}
	}
	function buildReprocTextCorrectFooterMCC() {
		return '</and></conditionvar><setvar action="Set"
			variablename="SCORE">SCORE.max</setvar><displayfeedback feedbacktype="Response"
			linkrefid="correct"/></respcondition>';
	}
		...
		foreach ($this->questions as $question) {
			...
			$type = $this->getType($question);
				if ($type=='Multiple Choice' || $type=='Multiple Answer') {
					foreach ($question["elements"]["answers_and_comments"] as $choice) {
						$js .= 'newQuestion['.$choice["question_answer_id"].'] = undefined;';						
					}
				} else if ($type=='Short Response') {
					$js .= 'newQuestion.BBAnswerId = undefined;';
				}
				$js .= 'newQuestion.type = "' . $type . '";';
			$js .='matchupData['.$question["elements"]["question_id"].'] = newQuestion;';		
	...
	function restOfSA() {
		return '<flow
				class="RESPONSE_BLOCK"><response_str ident="response" rcardinality="Single" rtiming="No"><render_fib
				 charset="us-ascii" columns="127" encoding="UTF_8" fibtype="String" maxchars="0" maxnumber="0" minnumber="0"
				 prompt="Box" rows="3"/></response_str></flow></flow></presentation><resprocessing
			   scoremodel="SumOfScores"><outcomes><decvar defaultval="0.0" maxvalue="1.0" minvalue="0.0" varname="SCORE"
			  vartype="Decimal"/></outcomes><respcondition title="correct"><conditionvar/><setvar action="Set"
			   variablename="SCORE">SCORE.max</setvar><displayfeedback feedbacktype="Response"
			 ...
			 linkrefid="incorrect"/></respcondition></resprocessing><itemfeedback ident="correct" view="All"><flow_mat
			   class="Block"><flow_mat class="FORMATTED_TEXT_BLOCK"><material><mat_extension><mat_formattedtext
			 type="HTML"/></mat_extension></material></flow_mat></flow_mat></itemfeedback><itemfeedback ident="incorrect"
			  view="All"><flow_mat class="Block"><flow_mat
				  class="FORMATTED_TEXT_BLOCK"><material><mat_extension><mat_formattedtext
			 type="HTML"/></mat_extension></material></flow_mat></flow_mat></itemfeedback><itemfeedback ident="solution"
			  view="All"><solution feedbackstyle="Complete" view="All"><solutionmaterial><flow_mat
				   class="Block"><material><mat_extension><mat_formattedtext type="HTML"/></mat_extension></material></flow_mat></solutionmaterial></solution></itemfeedback></item>';
	}
		...
		$js .= '
			...
			SendToCW.prototype = {
				...
				addBBMatchups: function() {
					...
					var SRQuestionDivs = document.getElementsByName("questionIdCWShortResponse");
					
					for (var divIdx=0; divIdx<SRQuestionDivs.length; divIdx++) {
						var questionIdDiv = SRQuestionDivs[divIdx];
						var questionId = questionIdDiv.innerHTML;
						var labelDivs = questionIdDiv.parentElement.parentElement.parentElement.getElementsByTagName("label");
						for (var labelDivIdx=0; labelDivIdx<labelDivs.length; labelDivIdx++) {
							var labelDiv = labelDivs[labelDivIdx];
							if (labelDiv.innerHTML == "Answer") {
								var BBAnswerId = labelDiv.getAttribute("for");
								matchupData[questionId].BBAnswerId = BBAnswerId;
							}
						}
					};
				},  
				...
				funcToAppend: function() {
					for (var questionId in matchupData) {
						...
						if (question.type == "Short Response") {
							var BBAnswerId = question.BBAnswerId;
							var answerType = "question_shortanswer";
							var sendCommand = "send_shortanswer";
							console.log("Answer Id is " + BBAnswerId);
							var answerElement = document.getElementById(BBAnswerId);
							if (answerElement) {
								var answerValue = this.toBase64(answerElement.value);
							} else {
								doSend = false;
								console.log("Failed to get answer for " + questionId);
							}
						} else if (question.type == "Multiple Choice" || question.type == "Multiple Answer") {
							var sendCommand = "send_answer_mc";
							var answerType = "question_answer_ids";
							for (var answerId in question) {
								var BBAnswerId = question[answerId];
								var answerElement = document.getElementById(BBAnswerId);
								if (answerElement && answerElement.checked) {
									answerValue = answerValue + "|" + answerId;
								};
							};
							if (answerValue[0]=="|") {
						...
						if (doSend) {
							this.sendAnswer(questionId, answerValue, sendCommand, answerType);
						}
					}
				},
				sendAnswer: function(questionId, answerValue, sendCommand, answerType) {
					var nameHash = hex_md5(this.name);
					var URL = "http://jimi.cbee.oregonstate.edu/concept_warehouse/bill_test/CW.php?goto=clicker&command=" + sendCommand + "&receiver="+ ' . $_SESSION["faculty_id"] . '+"&saved_test_association_id="+this.testId+"&clicker_type=blackboard&question_id="+questionId+"&clicker_id=" + nameHash + "&" + answerType + "=" + answerValue;
